...
const ContactItem = ({
  ...
  const renderIcon = () => {
    ...
    // Header in Creative is weird. It has a colored block then white section.
    // Contact info is usually in the white section in Creative Template.
    // So 'forceWhiteIcon' might not be used often unless we move contact to the top block.
    // We'll keep logic just in case.
...
const SectionHeading = ({ title, icon: Icon, formatting }) => {
  ...
  const style = formatting.heading?.style || "underline"; // Default for Creative
  const caps = formatting.heading?.caps || "uppercase"; // Creative usually uppercase
  ...
  // Default / Underline (Creative specific tweak: slightly thicker or different)
  // But let's stick to standard logic for parity
  return (
    <div className={cx("mb-3 flex", alignClass)}>
      {/* Creative Default: often has an accent separator bar before title */}
      <div
...
const TimelineItem = ({
  ...
  return (
    <div
      className="relative p-4 pb-4 last:pb-0 border rounded-xl shadow-md overflow-hidden"
      style={{ borderColor: `${accent}35`, background: `linear-gradient(180deg, ${accent}05, transparent)` }}
    >
      {/* Top accent bar */}
      <div
...
const CreativeTemplate = ({ data, formatting = {}, accentColor }) => {
  const effectiveFormatting = {
    ...formatting,
    colors: {
      ...formatting.colors,
      accent: formatting.colors?.accent || accentColor || "#8B5CF6",
    },
  };
  ...
  const secondaryColor = effectiveFormatting.colors?.secondary || "#4B5563";

  const currentFont = effectiveFormatting.font || {};
  const fontFamily = getFontFamily(currentFont) || "font-sans";
  const selectedFont = currentFont.family || "Poppins";
  const fontStyle = {
  ...
  React.useEffect(() => {
    if (selectedFont) {
      const linkId = "dynamic-font-loader-creative";
  ...
  const renderSectionContent = (id) => {
    switch (id) {
      case "summary":
        ...
        return (
          <div
            className="mb-8 p-4 bg-gray-50 rounded-lg border-l-4 border-accent"
      ...
      case "volunteer":
        ...
        return data.volunteer_experience.map((item, i) => {
          const mappedItem = { ...item, title: item.role || item.title };
          return <TimelineItem key={i} item={mappedItem} accent={accent} />;
        });
      case "skills":
  ...
  const SectionWrapper = ({ id }) => {
    // Unique for Creative: Summary is handled separately at top, or we can skip it here if rendered manually.
    // We already rendered Summary in renderSectionContent specialized block or we'll filter it out if we want it top-level.
    if (id === "summary") return renderSectionContent("summary"); // Just render it directly as a block
  ...
  const renderNodes = () => {
    if (isTwoColumn) {
      visibleSections.forEach((id) => {
        ...
        // Creative defaults: Skills, Education, Languages on right/sidebar usually?
        // "Right" in Creative was traditionally sidebar.
  ...
  const personalPositionRaw =
    personal.position || effectiveFormatting.section_positions?.personal || "header";
  ...
  const PersonalBlock = () => (
    <div className="space-y-3">
      ...
      <div className="h-px w-full" style={{ backgroundColor: `${secondaryColor}30` }} />
    </div>
  );
    ...
    >
      {!movePersonalToSidebar && (
        <div className="relative mb-0">
          <div
            className="h-40 flex items-center justify-center text-center px-4"
      ...
      >
        {!movePersonalToSidebar && (
          <>
          {/* Contact Bar - Centered & Clean */}
          <div className="flex flex-wrap justify-center gap-x-6 gap-y-3 mb-8 pb-6 border-b-2 border-gray-100 text-sm">
            {contactItems.map((item, i) => (
              <ContactItem
                key={i}
                icon={item.icon}
