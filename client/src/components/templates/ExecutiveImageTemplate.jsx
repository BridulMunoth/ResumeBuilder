...
const ContactItem = ({
  ...
  return (
    <div
      ...
      style={{ fontSize: "0.9em", color: forceWhiteIcon ? "white" : "inherit" }}
    >
...
const SectionHeading = ({ title, icon: Icon, formatting }) => {
  const accent = formatting.colors?.accent || "#1f2937";
  ...
  return (
    <div className={cx("mb-3 flex", alignClass)}>
      <div
        ...
        style={{ borderBottom: `2px solid ${accent}`, fontSize: fontSizeVal }}
      >
        {renderIcon()}
...
const ExecutiveImageTemplate = ({ data, formatting = {}, accentColor }) => {
  ...
  React.useEffect(() => {
    if (selectedFont) {
      const linkId = "dynamic-font-loader-executive-img";
  ...
  const PersonalBlock = () => (
    <div className="space-y-3">
      {imageSrc && (
        <div className="flex justify-start">
          <img
            src={imageSrc}
            alt="Profile"
            className="w-28 h-28 object-cover rounded-md shadow-lg border-2"
            style={{ borderColor: accent }}
          />
        </div>
      )}
      <h1 className="text-2xl font-bold" style={{ color: effectiveFormatting.colors.primary || "#111827" }}>
        {data.name || data.personal_info?.full_name || "Your Name"}
      </h1>
      {(data.role || data.personal_info?.profession) && (
        <p className="font-light" style={{ color: effectiveFormatting.colors.secondary || "#4B5563" }}>
          {data.role || data.personal_info?.profession}
        </p>
      )}
      ...
      <div className="h-px w-full" style={{ backgroundColor: `${effectiveFormatting.colors.secondary}30` }} />
    </div>
  );
  ...
  const getImageSrc = () => {
    if (headerConfig.imageAlign === "hidden") return null;
    const img = data.image || data.personal_info?.image;
    if (!img) return null;
  ...
  return (
    <div
      className={cx("bg-gray-50 min-h-[inherit] resume-font-root", fontFamily)}
      style={{
        ...fontStyle,
    ...
    >
      ...
      <div
        className="bg-white border-b shadow-sm relative overflow-hidden"
        style={{ borderTop: `6px solid ${accent}` }}
      >
        <div style={{ padding: contentPadding }}>
          <div
            className={`flex flex-col md:flex-row gap-8 items-center ${
              headerConfig.imageAlign === "right" ? "md:flex-row-reverse" : ""
            } ${
              headerConfig.imageAlign === "center"
          ...
          >
            ...
            >
              <h1
                ...
                style={{
                  color: effectiveFormatting.colors.primary || "#111827",
                }}
              >
                {data.name || data.personal_info?.full_name || "Your Name"}
              </h1>
