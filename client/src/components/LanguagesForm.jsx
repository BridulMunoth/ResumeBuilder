...
const LanguagesForm = ({ data = [], onChange }) => {
  ...
  // glassy input (same as SkillsForm)
  ...
  return (
    <div className="w-full rounded-3xl bg-gradient-to-br from-sky-100 via-white to-indigo-100 p-[1px] shadow-lg">
      <div className="space-y-6 rounded-3xl border border-white/60 bg-white/75 px-5 py-5 shadow-[0_18px_45px_rgba(15,23,42,0.16)] backdrop-blur-2xl sm:px-6 sm:py-6">
        ...
        {/* Added languages */}
        {(data || []).length > 0 ? (
          <div className="rounded-2xl border border-white/70 bg-white/55 p-3.5 shadow-inner backdrop-blur-md">
            <div className="flex flex-wrap gap-2">
              {data.map((lang, index) => (
                <span
                  key={index}
                  className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-sky-100/95 to-indigo-100/95 px-3.5 py-1.5 text-xs font-medium text-slate-800 shadow-sm ring-1 ring-white/70 backdrop-blur"
                >
                  <span>{lang?.name}</span>
                  ...
                  <button
                    ...
                    className="ml-1 rounded-full p-0.5 text-slate-500 transition hover:bg-white/80 hover:text-slate-900"
                  >
                    <X className="h-3 w-3" />
                  </button>
                </span>
        ...
        <div className="rounded-2xl bg-gradient-to-r from-blue-50/95 via-sky-50/95 to-indigo-50/95 px-3.5 py-3 shadow-sm backdrop-blur">
          <p className="text-sm text-slate-800">
            <strong className="font-semibold text-blue-700">Tip:</strong> Use
            clear levels like &quot;Fluent&quot; or &quot;Conversational&quot;. Add proofs (IELTS,
            JLPT, etc.) elsewhere if needed.
          </p>
        </div>
